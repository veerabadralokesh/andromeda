...
impl Solution {
    pub fn smallest_chair(times: Vec<Vec<i32>>, target_friend: i32) -> i32 {
        ...
        let mut times_friend = times.iter().enumerate().collect::<Vec<_>>();
        times_friend.sort_unstable_by_key(|tf| tf.1[0]);
        let mut chairs:BinaryHeap<Reverse<usize>> = BinaryHeap::with_capacity(n);
        for i in 0..n {
            chairs.push(Reverse(i));
        }
        let mut leaving_times:BinaryHeap<Reverse<i32>> = BinaryHeap::with_capacity(n);
        let mut leaving_time_to_chair:HashMap<i32, Vec<usize>> = HashMap::new();
        let mut alloted_chair = 0;
        for tf in times_friend.iter() {
            while !leaving_times.is_empty() && leaving_times.peek().unwrap().0 <= tf.1[0] {
                let leaving_time = leaving_times.pop().unwrap().0;
                for &chair in leaving_time_to_chair.get(&leaving_time).unwrap() {
                    chairs.push(Reverse(chair));
                }
                leaving_time_to_chair.remove(&leaving_time);
            }
            alloted_chair = chairs.pop().unwrap().0;
            if tf.0 == target_friend {
                // println!("{:?}", chairs);
                // println!("{:?}", leaving_times);
                // println!("{:?}", leaving_time_to_chair);
                return alloted_chair as _;
            }
            if leaving_time_to_chair.contains_key(&tf.1[1]) {
                leaving_time_to_chair.get_mut(&tf.1[1]).unwrap().push(alloted_chair);
            } else {
                leaving_times.push(Reverse(tf.1[1]));
                leaving_time_to_chair.insert(tf.1[1], vec![alloted_chair]);
            }
        }
        0
    }
